...
app = FastAPI()\
\
# Resolve local paths using absolute directory location\
BASE_DIR = os.path.dirname(os.path.abspath(__file__))\
templates = Jinja2Templates(directory=os.path.join(BASE_DIR, "templates"))\
security = HTTPBasic()\
\
# Core Site Configurations (Override via environment variables)\
ADMIN_USERNAME = os.getenv("ADMIN_USER", "admin")\
ADMIN_PASSWORD = os.getenv("ADMIN_PASS", "SecretAdmin2026!")\
PROXY_URL = os.getenv("PROXY_URL", "") # Premium residential proxy string goes here\
\
YOUTUBE_REGEX = r'^(https?://)?(www\\.)?(youtube\\.com|youtu\\.be)/.+$'\
DB_FILE = os.path.join(BASE_DIR, "analytics.db")\
\
# In-memory maps tracking user structural processing loads\
...
def authenticate_admin(credentials: HTTPBasicCredentials = Depends(security)):\
    if credentials.username != ADMIN_USERNAME or credentials.password != ADMIN_PASSWORD:\
        raise HTTPException(\
            ...
            detail="Unauthorized admin panel access attempt.",\
...
async def fetch_info(request: Request, url: str = Form(...)):\
    ...
    if not check_rate_limit(client_ip, fetch_ratelimit_store, max_requests=5):\
        raise HTTPException(status_code=429, detail="Lookup limit reached. Wait 60 seconds.")\
\
    if not re.match(YOUTUBE_REGEX, url):\
        raise HTTPException(status_code=400, detail="Invalid YouTube URL construction.")\
    \
    video_id = extract_video_id(url)\
    if is_blocked(video_id):\
        raise HTTPException(status_code=403, detail="This content streaming payload has been banned by node administration.")\
    \
    ...
    try:\
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:\
            ...
            \
            if not formats:\
                raise HTTPException(status_code=404, detail="No cross-platform extensions found.")\
                \
            return JSONResponse(\{\
                "title": str(raw_title),\
        ...
        raise HTTPException(status_code=500, detail="Internal parsing pipeline structural crash.")\
\
@app.get("/download")\
async def proxy_download(request: Request, url: str, filename: str, ext: str, quality: str):\
    ...
    if not check_rate_limit(client_ip, download_ratelimit_store, max_requests=3):\
        raise HTTPException(status_code=429, detail="Download volume threshold exceeded.")\
\
    try:\
        conn = sqlite3.connect(DB_FILE)\
        cursor = conn.cursor()\
        ...
        print(f"Log allocation failure error: \{e\}")\
\
...
\
        ...
        raise HTTPException(status_code=500, detail="The transmission engine lost peer streaming pipeline context.")\
\
# --- ADMINISTRATIVE APIS ---\
\
...
async def get_admin_stats(authenticated: bool = Depends(authenticate_admin)):\
    ...
    blocklist = [r[0] for r in blocked_rows]\
    \
    # FIXED: Explicit property indexing avoids rendering database tuple artifacts on UI\
    logs = [\{\
        "timestamp": str(row[0]), \
        "title": str(row[1]), \
        "quality": str(row[2]), \
